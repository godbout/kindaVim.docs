...
class SucceedingASNM_dE__Tests: SucceedingASNM_BaseTests {
    
    private func applyKeyCombinationsBeingTested(pgR: Bool = false) {
        kindaVimEngine.handle(keyCombination: KeyCombination(vimKey: .eight))
        kindaVimEngine.handle(keyCombination: KeyCombination(key: .d))
        kindaVimEngine.handle(keyCombination: KeyCombination(vimKey: .E), appMode: pgR == true ? .pgR : .auto)
    }
...
extension SucceedingASNM_dE__Tests {
    ...
    func test_that_in_Auto_Mode_it_calls_the_correct_function_on_AS_with_PGR_off() {
        ...
        XCTAssertEqual(asNormalModeMock.functionCalled, "dE(on:pgR:)")
