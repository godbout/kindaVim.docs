...
class SucceedingASNM_ciRightParenthesis_Tests: SucceedingASNM_BaseTests {
    ...
    private func applyKeyCombinationsBeingTested(pgR: Bool = false) {
        ...
        kindaVimEngine.handle(keyCombination: KeyCombination(vimKey: .rightParenthesis), appMode: pgR == true ? .pgR : .auto)
    }
...
extension SucceedingASNM_ciRightParenthesis_Tests {
    ...
    func test_that_in_Auto_Mode_it_calls_the_correct_function_on_AS_with_PGR_off() {
        ...
        XCTAssertEqual(asNormalModeMock.functionCalled, "ciRightParenthesis(on:pgR:_:)")
