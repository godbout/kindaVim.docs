...
class SucceedingASNM_ciRightBrace_Tests: SucceedingASNM_BaseTests {
    ...
    private func applyKeyCombinationsBeingTested(pgR: Bool = false) {
        ...
        kindaVimEngine.handle(keyCombination: KeyCombination(vimKey: .rightBrace), appMode: pgR == true ? .pgR : .auto)
    }
...
extension SucceedingASNM_ciRightBrace_Tests {
    ...
    func test_that_in_Auto_Mode_it_calls_the_correct_function_on_AS_with_PGR_off() {
        ...
        XCTAssertEqual(asNormalModeMock.functionCalled, "ciRightBrace(on:pgR:_:)")
