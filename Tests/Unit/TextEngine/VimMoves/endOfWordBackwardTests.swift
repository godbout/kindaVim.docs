...
extension endOfWordBackwardTests {
    ...
    func test_that_if_the_caret_is_after_the_last_character_on_a_non_empty_line_then_it_still_goes_to_the_end_of_the_last_word() {
        let text = """
a couple of
lines but not
coke haha
"""
        ...
        let newCaretPosition = textEngine.endOfWordBackward(startingAt: 35, in: TextEngineText(from: text))
...
extension endOfWordBackwardTests {
    ...
    func test_that_it_passes_several_consecutive_punctuations() {
        ...
        let newCaretPosition = textEngine.endOfWordBackward(startingAt: 13, in: TextEngineText(from: text))
